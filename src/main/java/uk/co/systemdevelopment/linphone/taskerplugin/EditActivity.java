    ...
    public void finish()
    {
        ...
        else
        {
            ...
            resultIntent.putExtra(com.twofortyfouram.locale.Intent.EXTRA_STRING_BLURB, 
            		String.format("%s account with index %d", deactivate ? " Deactivate" : "Activate", account_id));

            setResult(RESULT_OK, resultIntent);
			
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
			{
				int permissionCheck = getApplicationContext().checkSelfPermission(PERM_NAME_USE);
				if (permissionCheck == PackageManager.PERMISSION_DENIED)
				{
					ActivityCompat.requestPermissions(this, new String[]{PERM_NAME_USE}, 123);
				}
			}
        }
